...
def load_dict():
        ...
        map_dict=np.load('dataset\\modified\\map_dict.npy').item()
        return (index_vocab_dict,vocab_index_dict,map_dict)
...
def preprocess_data(dataset, vocab_index_dict, map_dict , Ty):

    Y = dataset

    #Y=[vocab_index_dict[str(t)]  for t in Y ]
    Y = [map_dict[i] for i in Y]
    Y = [string_to_int(t, Ty, vocab_index_dict) for t in Y]
...
def string_to_int(string, length, vocab):
    ...
    rep = list(map(lambda x: vocab.get(x, 0), string)) #10 : <unk>
